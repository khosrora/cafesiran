...
const LiveRegister = () => {
    ...
    const { userDetails, global } = useSelector(state => state);
    const [cities, setCities] = useState([]);
    const [provinceSelect, setProvince] = useState();


    const isCafeUser = userDetails.user?.cafe;
    const load = global.load;

    return (
        <>
            <Head>
                <title>منو دیجیتال ارزان |‌ دریافت سریع منو دیجیتال|‌ کافه ایران</title>
            ...
            <Layout>
                <div className="px-8 max-w-[1800px] m-auto mb-8">
                    <h1 className="text-xl mt-4 w-3/4 lg:m-auto dark:text-white">فرم ثبت سریع مجموعه</h1>
                    <Formik
                        initialValues={{
                            persian_title: '',
                            english_title: '',
                            ...
                            <Form className="mt-8 flex flex-col gap-y-8 m-auto lg:w-3/4" action="">
                                <div className="flex flex-col justify-between gap-y-8 text-slate-600 lg:flex-row lg:justify-between dark:text-white">
                                    <div className="flex flex-col justify-start items-start gap-y-2 lg:w-3/6">
                                        <label className="" htmlFor="mobile">شماره تماس مجموعه <span className="text-xs text-red-600">(شماره تماس برای ورود)</span></label>
                                ...
                                </div>
                                {
                                    load ?
                                        <div className="flex justify-between items-center">
                                            <p className="bg-[#FF7129] w-48 px-4 py-2 rounded-md text-white">
